...
func _on_area_2d_input_event(viewport, event, shape_idx):
	...
	if PuzzleVar.active_piece: #this fixes the multiple click issue
		...
		if Input.is_action_just_released("click"):
			if velocity == Vector2(0,0): #add mouse velocity as well and see if that helps with the tiny gaps, also see if maybe the issue is in the snap and connect code
